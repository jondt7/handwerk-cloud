...
export async function generateStaticParams() {
  const params: Array<{ locale: L; topic: string }> = [];
  for (const loc of locales) {
    const all = await getAllNews(loc as NewsLocale);
    const topics = Array.from(new Set(all.flatMap((p) => p.meta.topics || [])));
    params.push(...topics.map((t) => ({ locale: loc, topic: t })));
  }
  return params;
}

export async function generateMetadata({ params }: { params: Promise<{ locale: L; topic: string }> }) {
  const { locale, topic } = await params;
  const title = locale === 'de' ? `Thema: ${topic} – News` : `Topic: ${topic} – News`;
  const description = locale === 'de'
    ? `Aktuelle Meldungen zum Thema ${topic} – kuratiert und neutral zusammengefasst.`
    : `Latest headlines on ${topic} – curated and concise.`;
  return { title, description, alternates: { canonical: `/${locale}/topics/${encodeURIComponent(topic)}` } };
}
...
export default async function TopicPage({
  ...
  params: Promise<{ locale: L; topic: string }>;
  ...
  const filtered = all.filter((p) => (p.meta.topics || []).includes(topic));
  const total = filtered.length;
  const pages = Math.max(1, Math.ceil(total / pageSize));
  const slice = filtered.slice((page - 1) * pageSize, page * pageSize);

  const allTopics = Array.from(new Set(all.flatMap((p) => p.meta.topics || []))).sort();
  ...
  const labels = locale === 'de'
    ? { heading: `Thema: ${topic}`, none: 'Keine Meldungen gefunden.', back: 'Alle News' }
    : { heading: `Topic: ${topic}`, none: 'No news found.', back: 'All news' };

  const makeUrl = (n: number) => `/${locale}/topics/${encodeURIComponent(topic)}?page=${n}`;
        ...
        <div className="grid gap-8 md:grid-cols-3">
          ...
          <div>
            <NewsSidebar
              ...
              topTopics={allTopics.map((t) => ({ name: t, count: all.filter((x) => (x.meta.topics || []).includes(t)).length }))}
