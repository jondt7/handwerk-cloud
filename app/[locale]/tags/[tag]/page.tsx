...
export async function generateStaticParams() {
  const params: Array<{ locale: L; tag: string }> = [];
  for (const loc of locales) {
    const all = await getAllNews(loc as NewsLocale);
    const tags = Array.from(new Set(all.flatMap((p) => p.meta.tags || [])));
    params.push(...tags.map((t) => ({ locale: loc, tag: t })));
  }
  return params;
}
...
export default async function TagPage({
  params,
  searchParams
}: { params: Promise<{ locale: L; tag: string }>; searchParams: Promise<SP> }) {
  ...
  const filtered = all.filter((p) => (p.meta.tags || []).includes(tag));
  ...
  const labels = locale === 'de'
    ? { heading: `Tag: ${tag}`, none: 'Keine Meldungen gefunden.' }
    : { heading: `Tag: ${tag}`, none: 'No news found.' };

  const makeUrl = (n: number) => `/${locale}/tags/${encodeURIComponent(tag)}?page=${n}`;
